...
class MySQL {
	...
	function __construct() {
		$this->link = mysql_connect('127.0.0.1', 'username', 'password');
		if(!$this->link) throw new MySQLException(mysql_errno());
		if(!mysql_select_db('site_db')) throw new MySQLException('Could not select database.');
	}
